...
""" SSDH implementation: multiple fc_emb, multiple binary classifiers"""

class VGG16_SSDH2(VGG16):
    ...
    def _build_network(self, is_training=True):        
        ...
        with tf.variable_scope(self._scope, self._scope):
            ...
            for i in range(self._num_classes-1):                
                _fc_emb = self._image_encoding(fc7, is_training, scope="fc_emb"+str(i))
                _fc_emb = tf.reshape(_fc_emb, [_fc_emb.shape[0], 1, -1])
                fc_emb.append(_fc_emb)                                     
            
            fc_emb = tf.concat(fc_emb, axis=1, name="fc_emb")
    ...
    def _multi_image_classification(self, net, is_training):
        ...
        for i in range(self._num_classes-1):
            ...
            _fc_emb = net[:, i, :] # only if bottom is fc_emb concatenated (N, C, K)
            _fc_emb = tf.reshape(_fc_emb, [_fc_emb.shape[0], 1, 1, -1])

            _cls_score = slim.fully_connected(_fc_emb, 2, 
                          weights_initializer=initializer,
                          trainable=is_training,
                          activation_fn=None,
    ...
    def k1_euclidean_loss(self, norm_order=2, weights=1.0):
        ...
        for i in range(self._num_classes-1):
            _fc_emb = fc_emb[:, i, :]
            _fc_emb = tf.reshape(_fc_emb, [_fc_emb.shape[0],-1])
                
            _cal = tf.subtract(_fc_emb, 0.5)        
            _cal = tf.multiply(_cal, _cal)
            _cal = tf.reduce_mean(_cal, 1)
            _cal = tf.reduce_mean(_cal, 0)*(-1)
            _loss = tf.scalar_mul(weights, _cal)
            
            # add individual loss for each class
    ...
    def k2_euclidean_loss(self, norm_order=1, weights=1.0):
        ...
        for i in range(self._num_classes-1):            
            ...
            fc_avg = tf.reduce_mean(_fc_emb, 1)        
